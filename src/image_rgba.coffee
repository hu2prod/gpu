...
class @GPU_buffer_image_rgba
  ...
  load_buf_png : (buffer)->
    ...
    napi_png.png_decode_rgba buffer, @host
    @size_x = size_x
    @size_y = size_y
    @size = 4*size_x*size_y
    return
  
  load_buf_jpeg : (buffer)->
    [size_x, size_y] = napi_jpeg.jpeg_decode_size buffer
    @size_check size_x, size_y
    napi_jpeg.jpeg_decode_rgba buffer, @host
    @size_x = size_x
    @size_y = size_y
    @size = 4*size_x*size_y
    return
  ...
  save_buf_png : (buffer_reuse)->
    ...
    ret = napi_png.png_encode_rgba @host, @size_x, @size_y, buffer_reuse, 0
  ...
  save_buf_jpeg : (buffer_reuse, quality = 100)->
    if !buffer_reuse_orig = buffer_reuse
      buffer_reuse = util._buf_reuse or Buffer.alloc (@size_x*@size_y)//10
    ret = napi_jpeg.jpeg_encode_rgba @host, @size_x, @size_y, buffer_reuse, 0, quality
    if !buffer_reuse_orig
      util._buf_reuse = ret[2]
    ret
