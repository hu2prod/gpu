...
class @GPU_buffer_image_list_rgba
  ...
  init: (opt = {})->
    ...
    size = 4*@max_size_x*@max_size_y*@max_count
  ...
  load2idx_buf_png : (buffer, idx, skip_exact_size_check = false)->
    ...
    frame_size = 4*@size_x*@size_y
    dst = @host.slice(idx*frame_size, idx*frame_size + frame_size)
    napi_png.png_decode_rgba buffer, dst
    
    @count = Math.max @count, idx+1
    @size = 4*size_x*size_y*@count
    return
  ...
  load2idx_buf_jpeg : (buffer, idx, skip_exact_size_check = false)->
    ...
    frame_size = 4*@size_x*@size_y
    dst = @host.slice(idx*frame_size, idx*frame_size + frame_size)
    napi_jpeg.jpeg_decode_rgba buffer, dst
  ...
  load2idx_buf_raw : (buffer, idx)->
    ...
    frame_size = 4*@size_x*@size_y
    dst = @host.slice(idx*frame_size, idx*frame_size + frame_size)
    if buffer.length == rgba_size
      buffer.copy dst
    else
      # convert
      util.rgb2rgba buffer, dst
    @count = Math.max @count, idx+1
    @size = rgba_size*@count
    return
  ...
  save4idx_buf_png : (buffer_reuse, idx)->
    ...
    frame_size = 4*@size_x*@size_y
    src = @host.slice(idx*frame_size, idx*frame_size + frame_size)
    ret = napi_png.png_encode_rgba src, @size_x, @size_y, buffer_reuse, 0
  ...
  save4idx_buf_jpeg : (buffer_reuse, idx, quality = 100)->
    ...
    frame_size = 4*@size_x*@size_y
    src = @host.slice(idx*frame_size, idx*frame_size + frame_size)
    ret = napi_jpeg.jpeg_encode_rgba src, @size_x, @size_y, buffer_reuse, 0, quality
    if !buffer_reuse_orig
      util._buf_reuse = ret[2]
    ret
